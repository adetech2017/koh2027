  ...
  <AdminLayout>
    <div class="max-w-2xl">
      <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-dark">Edit Platform Pillar</h2>
      </div>
...
const pillar = computed(() => page.props.pillar || {})
...
watch(pillar, (newPillar) => {
  if (newPillar && newPillar.id) {
    form.title = newPillar.title || ''
    form.summary = newPillar.summary || ''
    form.body = newPillar.body || ''
    form.icon = newPillar.icon || ''
    form.color = newPillar.color || ''
    form.is_active = newPillar.is_active || false
    form.sort_order = newPillar.sort_order || 0
  }
}, { immediate: true, deep: true })

const submitForm = () => {
  errors.value = {}
  router.put(`/admin/platform-pillars/${pillar.value.id}`, form, {
    onError: (err) => {
      errors.value = err
    },
  })
